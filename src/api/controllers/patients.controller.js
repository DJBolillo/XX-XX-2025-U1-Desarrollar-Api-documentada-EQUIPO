...
async function confirmAppointmentStatus(req, res) {
  try {
    ...
    if (error.message.includes("ID de paciente inválido")) {
      return res.status(400).json({ message: "ID de paciente inválido" });
    } else if (error.message.includes("Cita no encontrada")) {
      ...
      res.status(500).json({ error: "Ocurrió un error al intentar cambiar su confirmación. Intente nuevamente más tarde." });
    }
  }
}
